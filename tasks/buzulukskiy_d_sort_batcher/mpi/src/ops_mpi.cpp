...
void RadixSortLSD(std::vector<int>& data) {
    ...
    const long long minVal = static_cast<long long>(
...
void ExchangeAndMerge(std::vector<int>& local, int partner, 
    ...
    MPI_Sendrecv(local.data(), static_cast<int>(local.size()), 
                MPI_INT, partner, 0,
                remote.data(), static_cast<int>(remote.size()), 
                MPI_INT, partner, 0,
    ...
    if (rank < partner) {
        local.assign(combined.begin(), 
                    combined.begin() + static_cast<std::ptrdiff_t>(myCount));
    } else {
        local.assign(combined.end() - static_cast<std::ptrdiff_t>(myCount), 
                    combined.end());
    }
}

void BatcherStabilizationPhase(std::vector<int>& local, int rank, int size, 
...
void BatcherNetworkPhase(std::vector<int>& local, int rank, int size, 
                        const std::vector<int>& counts) {
...
CalculateDistribution(int n, int size, int rank) {
...
bool BuzulukskiyDSortBatcherMPI::RunImpl() {
    ...
    MPI_Scatterv(rank == 0 ? GetInput().data() : nullptr, 
                 ...
                 local.data(), static_cast<int>(localSize), 
    ...
    MPI_Allgatherv(local.data(), static_cast<int>(local.size()), MPI_INT,
